...
class PlatformSimulator:
    ...
    def format_message(cls, platform: str, message: str):
        ...
        info = cls.PLATFORMS.get(
            platform, {"emoji": "❓", "color": "\033[97m", "name": platform}
        )
...
async def demo_basic_messaging():
    ...
    try:
        ...
        await asyncio.gather(
            code.connect(), browser.connect(), desktop.connect(), vscode.connect()
        )
        ...
        # Code → All
        print(
            PlatformSimulator.format_message(
                "code",
        ...
        print(
            PlatformSimulator.format_message(
                "desktop", "I can check the logs. What's the error message?"
            )
        )
...
async def demo_think_tank_cross_platform():
    ...
    try:
        ...
        print(
            PlatformSimulator.format_message(
                "code", "DECISION: Use REST API for microservices communication"
            )
        )
        decision = await code.propose_decision(
            "Use REST API for microservices communication", vote_type="consensus"
        )
        ...
        print(
            PlatformSimulator.format_message(
                "desktop", "👍 PRO: gRPC has strong typing with Protocol Buffers"
            )
        )
        ...
        print(
            PlatformSimulator.format_message(
                "vscode", "👎 CON: gRPC has steeper learning curve for team"
            )
        )
        ...
        print(
            PlatformSimulator.format_message(
                "vscode", "AMENDMENT: Add training budget for gRPC upskilling"
            )
        )
        ...
        print(PlatformSimulator.format_message("code", "Accepting amendment..."))
...
async def demo_request_response_pattern():
    ...
    try:
        ...
        print(
            PlatformSimulator.format_message(
                "code", "Can you search the web for the latest Claude API pricing?"
            )
        )
        await code.send_message(
...
async def demo_broadcast():
    ...
    try:
        ...
        print(
            PlatformSimulator.format_message(
                "code", "🚨 URGENT: Production database is down! All hands on deck!"
            )
        )
        await code.send_message(
        ...
        print(
            PlatformSimulator.format_message(
                "browser", "Received! Checking status dashboards..."
            )
        )
        print(PlatformSimulator.format_message("desktop", "Received! Checking logs..."))
        print(
            PlatformSimulator.format_message(
                "vscode", "Received! Looking at recent code changes..."
            )
        )
