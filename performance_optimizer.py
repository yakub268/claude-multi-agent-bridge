...
class PerformanceOptimizer:
    ...
    def get_cached_route(
        self, from_client: str, to_client: str
    ) -> Optional[CachedRoute]:
        """
        Get cached route
    ...
    def cache_route(
        self, from_client: str, to_client: str, route_type: str, latency_ms: float
    ):
        """
        ...
        if len(self.route_cache) > 1000:
            ...
            sorted_routes = sorted(
                self.route_cache.items(), key=lambda x: x[1].last_used
            )
            for key, _ in sorted_routes[:100]:
                del self.route_cache[key]

    def should_batch(
        self, to_client: str, payload_size: int, batch_threshold: int = 1024
    ) -> bool:
        """
    ...
    def flush_batch(self, to_client: str, send_func: Callable) -> int:
        """
        ...
        with self.batch_lock:
            ...
            # Send batch
            batch = {"type": "batch", "count": len(messages), "messages": messages}
    ...
    def auto_flush_batches(self, send_func: Callable, max_wait_ms: int = 100):
        """
    ...
    def is_fast_path_eligible(self, message: Dict) -> bool:
        ...
        fast_types = {"heartbeat", "ping", "status_update", "metrics", "telemetry"}
    ...
    def should_compress(self, payload: Dict, threshold_bytes: int = 1024) -> bool:
        """
