...
try:
    ...
    logger.warning(
        "Collaboration features not available (collab_ws_integration.py missing)"
    )
...
MAX_CONNECTIONS_PER_CLIENT = int(
    os.getenv("MAX_CONNECTIONS_PER_CLIENT", "10")
)  # Per client ID
...
def websocket_handler(ws, client_id):
    ...
    logger.info(
        f"WebSocket connection opened: {client_id} (conn_id: {connection_id[:8]})"
    )
    ...
    if total_connections >= MAX_CONNECTIONS:
        logger.warning(
            f"Connection limit reached ({total_connections}/{MAX_CONNECTIONS}), rejecting {client_id}"
        )
        ws.send(
            json.dumps(
                {
    ...
    if client_connections >= MAX_CONNECTIONS_PER_CLIENT:
        logger.warning(
            f"Per-client connection limit reached for {client_id} ({client_connections}/{MAX_CONNECTIONS_PER_CLIENT})"
        )
        ws.send(
            json.dumps(
                {
    ...
    try:
        ...
        while True:
            ...
            if current_time - last_ping_time >= ping_interval:
                try:
                    ws.send(json.dumps({"type": "ping", "timestamp": utc_timestamp()}))
        ...
        if conn_info:
            logger.info(
                f"WebSocket connection closed: {client_id} (conn_id: {conn_info['connection_id'][:8]})"
            )
        ...
        metrics["active_connections"] = sum(
            len(conns) for conns in ws_connections.values()
        )
...
def handle_ws_message(ws, client_id, message):
    ...
    if msg_type == "send":
        ...
        # Respond to ping
        ws.send(json.dumps({"type": "pong", "timestamp": utc_timestamp()}))
        ...
        if collab_bridge:
            ...
            ws.send(
                json.dumps(
                    {"type": "error", "error": "Collaboration features not available"}
                )
            )
...
def get_status():
    ...
    if metrics["last_message_time"]:
        last_time = datetime.fromisoformat(
            metrics["last_message_time"].replace("Z", "+00:00")
        )
        elapsed = (datetime.now(timezone.utc) - last_time).total_seconds()
        if elapsed > 0:
            metrics["messages_per_minute"] = round(
                metrics["total_messages"] / (elapsed / 60), 2
            )
    ...
    status = {
        ...
        "connections": {
            ...
            "by_client": {
                client: len(conns) for client, conns in ws_connections.items()
            },
        },
        "queue": {"current": len(message_store), "max": message_store.maxlen},
...
def list_collab_rooms():
    ...
    try:
        ...
        return jsonify({"status": "success", "rooms": rooms, "total": len(rooms)}), 200
    except Exception as e:
...
def get_collab_room(room_id):
    ...
    try:
        ...
        return jsonify({"status": "success", "room": summary}), 200
    except Exception as e:
...
def graceful_shutdown(signum, frame):
    ...
    logger.info(
        f"Closing {sum(len(conns) for conns in ws_connections.values())} WebSocket connections..."
    )
...
if __name__ == "__main__":
    ...
    if COLLAB_ENABLED:
        logger.info(
            "  ✅ Collaboration rooms (enhanced voting, channels, code execution)"
        )
